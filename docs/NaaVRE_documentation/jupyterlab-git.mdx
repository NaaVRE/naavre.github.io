...
:::note
Currently the Jupyterlab-git feature has a bug breaking the clonining of a repo using the user interface.
In stead you can clone a repo using a terminal (available in the launcher). Open a terminal and execute the following command:

`git clone <URL e.g. https://github.com/NaaVRE/vl-openlab.git>`
:::
...
:::note
The push button in the git-extension is currently broken. You can use `Push to remote` in the Git menu in the items instead:

![push_to_remote.png](./images/push_to_remote.png)
:::
