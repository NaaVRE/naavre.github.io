...
* Splitter: This component is used to split an output list or array into batches and feed each batch to the
  next workflow component. That way the next component will be executed in parallel as many times as the number of batches.
  You can limit the number of branches created by the splitter by using the pencil icon on the splitter.

![Max branches](images/max_branches.png)

* Merger: This component is used to merge the output list or array of the workflow component parallelized by the splitter into a single list or array.
...
:::important
Cells shared with a community or virtual lab are visible to everyone.
