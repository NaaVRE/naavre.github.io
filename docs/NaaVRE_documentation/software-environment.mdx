...
```
:::warning
Notebook cells that contain both `!pip install` commands and code break [the NaaVRE containerizer](./../creating-workflow-components).
Put `!pip install` commands in a separate cell from the code that uses the installed packages.
:::
