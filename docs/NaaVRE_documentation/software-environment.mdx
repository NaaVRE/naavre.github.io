...
```
:::warning
Notebook cells that contain both `!pip install` commands and code, disrupt [the NaaVRE containerizer](./../creating-workflow-components).
The containerizer assumes that cells containing code do not include any other commands.
Place `!pip install` commands in a separate notebook cell from the code that utilizes the installed packages.
:::
...
:::note
If you want to use your own code that is not yet packaged, you can include it in your workflow components by importing it from a file stored in cloud storage.
