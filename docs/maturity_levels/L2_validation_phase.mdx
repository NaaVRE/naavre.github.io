...
* Versioning
  ...
  * Give each workflow component and workflow a version number.
...
* Infrastructure
  * Check the infrastructure requirements for the workshop:
     * The number of people taking part in a workshop.
     * The random access memory and permanent storage usage of the virtual lab.
...
* Virtual lab developers: <Responsibilities.L2VirtualLabDeveloper/>
...
* Codebase
  ...
  - [ ]  The code (in juppyter notebooks) is easily human-readable and others can easily modify it. If methods have side effects, this is clear to the user.
  - [ ]  The input and output of each notebook cell is clear. It is both clear what the structure is (e.g. what data type is used) and what the data content is from a domain perspective.
* Workflow
...
* Publication
  - [ ] Research conducted in NaaVRE is submitted as a publication. See [dissemination guidelines](./dissemination.mdx).